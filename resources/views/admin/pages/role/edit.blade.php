            ...
            <form action="{{ route('admin.role.update', $role->id) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input id="name" name="name" value="{{ $role->name }}" type="text"
                            ...
                            <div class="col-lg-9" id="checkboxlist">
                                <div class="row">
                                    @foreach ($permissionNames as $permissionName)
                                        <div class="col-lg-4">
                                            <label><input class="me-1 groupPermission{{ $groupName->id }}" type="checkbox" name="permission_id[]" {{ $role->hasPermissionTo($permissionName->name) ? 'checked' : '' }}
                                                    value="{{ $permissionName->id }}" />{{ $permissionName->name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
